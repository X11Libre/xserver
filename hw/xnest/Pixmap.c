...
xnestPixmapToRegion(PixmapPtr pPixmap)
{
    ...
    xcb_get_image_cookie_t cookie = xcb_get_image(
        xnestUpstreamInfo.conn, XCB_IMAGE_FORMAT_XY_PIXMAP, xnestPixmap(pPixmap),
        0, 0, pPixmap->drawable.width, pPixmap->drawable.height, ~0);

    xcb_generic_error_t *err = NULL;
    xcb_get_image_reply_t *reply= xcb_get_image_reply(
        xnestUpstreamInfo.conn, cookie, &err);
