...
{
    xcb_xkb_use_extension_cookie_t cookie1 = xcb_xkb_use_extension(
    ...
    xcb_xkb_use_extension_reply_t *reply = xcb_xkb_use_extension_reply(
        xnestUpstreamInfo.conn, cookie1, &err);
    if (err) {
        free(err);
    } else {
        free(reply);
...
int xnestXkbDeviceId(xcb_connection_t *conn)
{
    ...
    uint8_t xlen[6] = { 0 };
    xcb_xkb_get_kbd_by_name_cookie_t cookie = xcb_xkb_get_kbd_by_name(
        xnestUpstreamInfo.conn,
