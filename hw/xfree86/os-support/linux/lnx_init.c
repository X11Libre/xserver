...
#include "../shared/seatd-libseat.h"
...
linux_parse_vt_settings(int may_fail)
{
    ...
    if (xf86Info.vtno != -1) {
        ...
        if (fd < 0) {
            if (may_fail)
                return 0;
            FatalError("parse_vt_settings: Cannot open /dev/tty0 (%s), maybe missing for ex. '-seat seat0' parameter? (in case trying to run in rootless mode)\n",
                       strerror(errno));
        }
