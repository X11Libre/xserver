...
xf86CrtcCreate(ScrnInfoPtr scrn, const xf86CrtcFuncsRec * funcs)
{
    ...
    if (xf86_config->crtc) {
      crtcs = reallocarray(xf86_config->crtc, xf86_config->num_crtc + 1,
                           sizeof(xf86CrtcPtr));
    } else {
...
xf86CrtcDestroy(xf86CrtcPtr crtc)
{
    ...
    for (c = 0; c < xf86_config->num_crtc; c++) {
      if (xf86_config->crtc[c] == crtc) {
        memmove(&xf86_config->crtc[c], &xf86_config->crtc[c + 1],
                 ...
                 const xf86OutputFuncsRec * funcs, const char *name)
{
    ...
    if (xf86_config->output) {
      outputs = reallocarray(xf86_config->output, xf86_config->num_output + 1,
...
xf86OutputDestroy(xf86OutputPtr output)
{
    ...
    for (o = 0; o < xf86_config->num_output; o++) {
      if (xf86_config->output[o] == output) {
        memmove(&xf86_config->output[o], &xf86_config->output[o + 1],
...
xf86ProbeOutputModes(ScrnInfoPtr scrn, int maxX, int maxY)
{
    ...
    for (o = 0; o < config->num_output; o++) {
        ...
        if (xf86GetOptValFreq(output->options, OPTION_MIN_CLOCK, OPTUNITS_KHZ,
                              &clock)) {
          min_clock = (int)clock;
        }
        if (xf86GetOptValFreq(output->options, OPTION_MAX_CLOCK, OPTUNITS_KHZ,
                              &clock)) {
          max_clock = (int)clock;
        }
...
xf86InitialConfiguration(ScrnInfoPtr scrn, Bool canGrow)
{
    ...
    if (ret == FALSE && canGrow) {
      ...
      } else if (xf86TargetRightOf(scrn, config, modes, enabled, width,
                                   height)) {
      ...
      } else if (xf86TargetAspect(scrn, config, modes, enabled, width,
                                  height)) {
        xf86DrvMsg(i, X_INFO, "Using fuzzy aspect match for initial modes\n");
      } else if (xf86TargetFallback(scrn, config, modes, enabled, width,
                                    height)) {
    ...
    for (o = -1; nextEnabledOutput(config, enabled, &o);) {
      if (!modes[o]) {
        ...
        xf86DrvMsg(scrn->scrnIndex, X_INFO,
                   ...
                   config->output[o]->initial_x, config->output[o]->initial_y);
      }
    }
    ...
    if (det_mon->type == DT &&
        ...
        if (((det_mon->section.d_timings.h_size * 5) <
          ...
          xf86DrvMsg(p->output->scrn->scrnIndex, X_WARNING,
                     ...
                     p->output->name, det_mon->section.d_timings.h_size,
