...
RenderExtensionInit(void)
{
    ...
    if (!dixRegisterPrivateKey(&RenderClientPrivateKeyRec, PRIVATE_CLIENT,
...
SingleRenderComposite(ClientPtr client, xRenderCompositeReq *stuff)
{
    ...
    if ((pSrc->pDrawable &&
         pSrc->pDrawable->pScreen != pDst->pDrawable->pScreen) ||
        (pMask && pMask->pDrawable &&
         pDst->pDrawable->pScreen != pMask->pDrawable->pScreen)) {
      return BadMatch;
    }
...
SingleRenderTrapezoids(ClientPtr client, xRenderTrapezoidsReq *stuff)
{
    ...
    if (ntraps) {
      CompositeTrapezoids(stuff->op, pSrc, pDst, pFormat, stuff->xSrc,
                          stuff->ySrc, ntraps, (xTrapezoid *)&stuff[1]);
    }
    return Success;
}
...
SingleRenderTriangles(ClientPtr client, xRenderTrianglesReq *stuff)
{
    ...
    if (ntris) {
      CompositeTriangles(stuff->op, pSrc, pDst, pFormat, stuff->xSrc,
                         stuff->ySrc, ntris, (xTriangle *)&stuff[1]);
    }
    return Success;
}
...
SingleRenderTriStrip(ClientPtr client, xRenderTriStripReq *stuff)
{
    ...
    npoints >>= 3;
    if (npoints >= 3) {
      CompositeTriStrip(stuff->op, pSrc, pDst, pFormat, stuff->xSrc,
                        stuff->ySrc, npoints, (xPointFixed *)&stuff[1]);
    }
    return Success;
}
...
SingleRenderTriFan(ClientPtr client, xRenderTriFanReq *stuff)
{
    ...
    npoints >>= 3;
    if (npoints >= 3) {
      CompositeTriFan(stuff->op, pSrc, pDst, pFormat, stuff->xSrc, stuff->ySrc,
...
SingleRenderAddTraps(ClientPtr client, xRenderAddTrapsReq *stuff)
{
    ...
    ntraps /= sizeof(xTrap);
    if (ntraps) {
      AddTraps(pPicture, stuff->xOff, stuff->yOff, ntraps, (xTrap *)&stuff[1]);
    }
    return Success;
}
...
ProcRenderCreateLinearGradient(ClientPtr client)
{
    ...
    if (client->swapped) {
        ...
        if (stuff->nStops >
            UINT32_MAX / (sizeof(xFixed) + sizeof(xRenderColor))) {
          return BadLength;
        }
        if (len != stuff->nStops * (sizeof(xFixed) + sizeof(xRenderColor))) {
          return BadLength;
